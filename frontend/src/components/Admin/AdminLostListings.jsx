...
function AdminLostListings() {
    ...
    const handleDeletePet = (deletedPetId) => {
        setListings(prev => prev.filter(listing => listing._id !== deletedPetId))
    }
    ...
    return (
        <div className="p-4">
            <h1 className="text-2xl font-bold mb-8">Kayıp İlanlar</h1>
            <ul>
                {currentItems.map(listing => (
                    <Pet key={listing._id} pet={listing} onDelete={handleDeletePet} isLost={true} />
                ))}
            </ul>
...
export default AdminLostListings
